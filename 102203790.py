...
def validate_inputs(weights, impacts, num_criteria):
    ...
    if len(weights) != num_criteria or len(impacts) != num_criteria:
        print(
            "Error: The number of weights, impacts, and criteria columns must be the same.")
        sys.exit(1)
...
def topsis(input_file, weights, impacts, output_file):
    ...
    ideal_best = np.where(np.array(impacts) == '+',
                          norm_data.max(), norm_data.min())
    ideal_worst = np.where(np.array(impacts) == '+',
